...
namespace wire {
    ...
    VulkanShaderResourceLayout::VulkanShaderResourceLayout(VulkanRenderer* renderer, const ShaderResourceLayoutInfo& layoutInfo)
        : m_Renderer(renderer)
    {
        for (const auto& set : layoutInfo.Sets)
        {
            ...
            VkResult result = vkCreateDescriptorSetLayout(m_Renderer->getDevice(), &createInfo, m_Renderer->getAllocator(), &setLayout);
    ...
    {
        for (VkDescriptorSetLayout setLayout : m_SetLayouts)
            vkDestroyDescriptorSetLayout(m_Renderer->getDevice(), setLayout, m_Renderer->getAllocator());
    }

    VulkanShaderResource::VulkanShaderResource(VulkanRenderer* renderer, uint32_t set, ShaderResourceLayout* layout)
        : m_Renderer(renderer)
    {
        ...
        VkResult result = vkAllocateDescriptorSets(renderer->getDevice(), &allocInfo, &m_Set);
