...
namespace wire {
    ...
    VulkanRenderPass::VulkanRenderPass(VulkanRenderer* renderer, VulkanSwapchain* swapchain, const RenderPassDesc& desc, std::string_view debugName)
        : m_Renderer(renderer), m_Swapchain(swapchain), m_Desc(desc), m_DebugName(debugName)
    {
        recreate();
    }

    VulkanRenderPass::VulkanRenderPass(VulkanRenderer* renderer, VulkanFramebuffer* framebuffer, const RenderPassDesc& desc, std::string_view debugName)
        : m_Renderer(renderer), m_Framebuffer(framebuffer), m_Desc(desc), m_DebugName(debugName)
    {
        recreate();
    }
    ...
    void VulkanRenderPass::recreate()
    {
        ...
        VkResult result = vkCreateRenderPass(m_Renderer->getDevice(), &renderPassInfo, m_Renderer->getAllocator(), &m_RenderPass);
        VK_CHECK(result, "Failed to create Vulkan render pass!");

        std::string renderPassName = m_DebugName + " (render pass)";
        VK_DEBUG_NAME(m_Renderer->getDevice(), RENDER_PASS, m_RenderPass, renderPassName.c_str());
        ...
        if (m_Swapchain)
        {
            ...
            {
                bool found = false;
                for (uint32_t i = 0; i < m_Swapchain->getDesc().Attachments.size(); i++)
                {
                    if (m_Swapchain->getDesc().Attachments[i] == rpAttachment.Format)
                    {
    ...
    {
        m_Renderer->submitResourceFree([framebuffers = m_Framebuffers, renderPass = m_RenderPass](Renderer* renderer)
        {
        ...
        if (m_Swapchain)
        {
            ...
            for (uint32_t i = 0; i < m_Swapchain->getImageCount(); i++)
            {
                ...
                VkResult result = vkCreateFramebuffer(m_Renderer->getDevice(), &framebufferInfo, m_Renderer->getAllocator(), &m_Framebuffers[i]);
                VK_CHECK(result, "Failed to create Vulkan framebuffer!");

                std::string framebufferName = m_DebugName + " (framebuffer " + std::to_string(i) + ")";
                VK_DEBUG_NAME(m_Renderer->getDevice(), FRAMEBUFFER, m_Framebuffers[i], framebufferName.c_str());
            }
        }
        else
        {
            ...
            VkResult result = vkCreateFramebuffer(m_Renderer->getDevice(), &framebufferInfo, m_Renderer->getAllocator(), &m_Framebuffers[0]);
            VK_CHECK(result, "failed to create Vulkan framebuffer");

            std::string framebufferName = m_DebugName + " (framebuffer)";
            VK_DEBUG_NAME(m_Renderer->getDevice(), FRAMEBUFFER, m_Framebuffers[0], framebufferName.c_str());
        }
    }
