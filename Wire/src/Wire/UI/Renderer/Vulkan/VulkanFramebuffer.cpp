...
namespace wire {
    ...
    VulkanFramebuffer::VulkanFramebuffer(Renderer* renderer, const FramebufferDesc& desc, std::string_view debugName)
    ...
    {
        m_Renderer->submitResourceFree([image = m_Image, view = m_View, memory = m_Memory, mips = m_Mips,
            depthImage = m_DepthImage, depthView = m_DepthView, depthMemory = m_DepthMemory](Renderer* renderer)
        {
    ...
    void VulkanFramebuffer::resize(const glm::vec2& extent)
    {
        ...
        {
            CommandList list = m_Renderer->beginSingleTimeCommands();
            list.imageMemoryBarrier(this, AttachmentLayout::Undefined, m_Desc.Layout, 0, m_Desc.MipCount);
            m_Renderer->endSingleTimeCommands(list);
        }
    }
