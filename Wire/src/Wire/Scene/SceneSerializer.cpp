...
namespace YAML {
	...
	template<>
	struct convert<Wire::UUID>
	{
		static Node encode(const Wire::UUID& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, Wire::UUID& rhs)
		{
...
#define READ_SCRIPT_FIELD(FieldType, Type)					\
	...
	static void SerializeEntity(YAML::Emitter& out, Entity entity)
	{
		...
		if (entity.HasComponent<ScriptComponent>())
		{
			...
			out << YAML::Key << "Class" << YAML::Value << name;
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		WR_CORE_TRACE("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								WR_CORE_ASSERT(fields.find(name) != fields.end());
				...
				if (circleRendererComponent)
				{
					...
					crc.Colour = circleRendererComponent["Colour"].as<glm::vec4>();
				...
				if (textComponent)
				{
					...
					// tc.FontAsset // TODO
					tc.Colour = textComponent["Colour"].as<glm::vec4>();
