...
#ifdef WR_DEBUG
#define WR_ASSERT(x, msg, ...) { if (!(x)) { WR_ERROR(msg, ##__VA_ARGS__); raise(SIGTRAP); } }

#define WR_ASSERT_OR_WARN(x, msg, ...) { if (!(x)) { WR_ERROR(msg, ##__VA_ARGS__); raise(SIGTRAP); } }
#define WR_ASSERT_OR_ERROR(x, msg, ...) { if (!(x)) { WR_ERROR(msg, ##__VA_ARGS__); raise(SIGTRAP); } }
#else
#define WR_ASSERT(...)
